...
function ensureControls(camera) {
  if (rendererState._controlsInitialized || !rendererState.renderer || !camera) return null;
  ...
  const onStart = () => { _userInteracted = true; _needsRender = true; };
  const onEnd = () => { _needsRender = true; };
  controls.addEventListener("start", onStart);
  controls.addEventListener("end", onEnd);

  rendererState.controls = controls;
  rendererState._controlsInitialized = true;
  return controls;
}

function syncControlsToCamera(camera) {
...
let _manualRenderActive = false;

function startRenderLoop() {
  if (rendererState._loopStarted) return;
  rendererState._loopStarted = true;

  function loop() {
    requestAnimationFrame(loop);
    const controls = rendererState.controls;
    if (_needsRender && _manualRenderActive === false) {
      _needsRender = false;
      if (controls) {
        controls.update();
      }
      rendererState.renderer.render(rendererState.scene, rendererState.camera);
    } else if (controls && controls.enabled && _manualRenderActive === false) {
      controls.update();
      rendererState.renderer.render(rendererState.scene, rendererState.camera);
    }
  }
...
export function renderMap(canvas, config) {
  ...
  if (!rendererState.controls || rendererState.controls.object !== camera) {
    ensureControls(camera);
  }
  syncControlsToCamera(camera);

  _manualRenderActive = true;
  const controls = rendererState.controls;
  if (controls) {
...
export function setupScrollZoom(canvas) {
  ...
  canvas.tabIndex = 0;
  canvas.title = "Scroll to zoom, drag to rotate, right-click to pan.";
}
