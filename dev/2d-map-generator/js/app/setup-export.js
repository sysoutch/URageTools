...
function generateSidescrollerMap() {
  ...
  for (var supportRow = 0; supportRow < sideTilesY; supportRow++) {
    for (var supportCol = 0; supportCol < sideTilesX; supportCol++) {
      ...
      if (sidePlacePlayers && !characterPlaced && canPlaceAbove) {
        baseItems[drawRow][supportCol] = "character";
  ...
  resizeCanvasForMap(outputSize.cols, outputSize.rows, sideStartX, sideStartY, stepX, stepY);
  clearAndDrawBackground();

  for (var y = 0; y < outputSize.rows; y++) {
    for (var x = 0; x < outputSize.cols; x++) {
      if (currentMap[y] && currentMap[y][x] === "platform") {
        drawBottomOriginTileAt(platformImg, x, y, outputSize.rows, sideStartX, sideStartY, stepX, stepY);
      } else if (holeImg && holeImg.src) {
        drawBottomOriginTileAt(holeImg, x, y, outputSize.rows, sideStartX, sideStartY, stepX, stepY);
      }
    }
  }
  ...
  for (var itemY = 0; itemY < outputSize.rows; itemY++) {
    for (var itemX = 0; itemX < outputSize.cols; itemX++) {
      ...
      if (itemId === "character") {
        drawBottomOriginSurfaceSprite(document.getElementById("characterRed"), itemX, itemY, outputSize.rows, sideStartX, sideStartY, stepX, stepY);
        continue;
      }
      var itemType = getSpriteType(itemId);
      if (itemType) {
        drawBottomOriginSurfaceSprite(getSpriteImage(itemType), itemX, itemY, outputSize.rows, sideStartX, sideStartY, stepX, stepY);
      }
    }
  }
